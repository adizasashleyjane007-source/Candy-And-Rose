...
export default function ServicePage() {
    ...
    const [services, setServices] = useState<{ id: number, name: string, category: string, price: string, duration: string }[]>([]);
    const [isLoaded, setIsLoaded] = useState(false);

    const defaultServices = [
        { id: 1, name: "Classic Haircut", category: "Hair", price: "₱150", duration: "30 mins" },
        { id: 2, name: "Gel Manicure", category: "Nails", price: "₱250", duration: "45 mins" },
        { id: 3, name: "Hair Coloring (Full)", category: "Hair", price: "₱1,500", duration: "2 hrs" },
        { id: 4, name: "Facial Spa", category: "Face", price: "₱800", duration: "1 hr" }
    ];
    ...
    const uniqueCategories = ["All Categories", ...Array.from(new Set(services.map(s => s.category)))];
                ...
                <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-pink-200 overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-separate" style={{ borderSpacing: "0 10px" }}>
                            <thead>
                                <tr>
                                    <th className="pb-2 px-4 text-xs font-bold text-pink-500 uppercase tracking-wider whitespace-nowrap">Service Name</th>
                                    <th className="pb-2 px-4 text-xs font-bold text-pink-500 uppercase tracking-wider whitespace-nowrap">Category</th>
                                    ...
                                    <th className="pb-2 px-4 text-xs font-bold text-pink-500 uppercase tracking-wider whitespace-nowrap w-28 text-center">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                {paginatedServices.map((svc) => (
                                    <tr key={svc.id} className="bg-gray-50/50 hover:bg-pink-50/50 transition-all shadow-sm group">
                                        <td className="py-3.5 px-4 text-sm font-semibold text-gray-900 rounded-l-xl border border-transparent group-hover:border-pink-200 border-r-0">{svc.name}</td>
                                        <td className="py-3.5 px-4 text-sm font-medium text-gray-600 border border-transparent group-hover:border-pink-200 border-x-0">{svc.category}</td>
                                        <td className="py-3.5 px-4 text-sm font-semibold text-gray-900 border border-transparent group-hover:border-pink-200 border-x-0">{svc.price}</td>
                                        <td className="py-3.5 px-4 text-sm font-medium text-gray-600 border border-transparent group-hover:border-pink-200 border-x-0">{svc.duration}</td>
                        ...
                        <form onSubmit={handleFormSave} className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-2 border border-pink-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-pink-500 text-gray-900"
